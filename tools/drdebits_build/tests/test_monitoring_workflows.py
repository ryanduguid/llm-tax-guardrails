...
def test_monitoring_reuses_legacy_alerts_and_recovers_all_reports(
        tmp_path, workflow, prefix, legacy, existing, recovery):
    issues = [{"number": 99, "title": prefix + ": unrelated issue"}]
    ...
    expected = sorted(issue["number"] for issue in issues if issue["number"] != 99)
...
gh() {
  case "$1 $2" in
    "issue list") printf '%s' "$FIXTURE" | "$JQ" -r "${!#}" ;;
...
CHECK_EXIT_CODE=2
'''
    setup = f"FIXTURE={shlex.quote(json.dumps(issues))}\nJQ={shlex.quote(Path(JQ).as_posix())}\n"
    ...
    assert all("99" not in operation for operation in operations)
